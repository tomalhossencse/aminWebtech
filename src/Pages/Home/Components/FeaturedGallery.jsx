...
const FeaturedGallery = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentSlide(
        (prev) => (prev + 1) % Math.ceil(galleryItems.length / 3),
      );
  ...
  return (
    <section
      ref={sectionRef}
      className="py-20 px-6 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-base-100"
    >
      {/* Section Header */}
      <div
      ...
      >
        <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight flex items-center justify-center gap-2 mb-4">
          <i className="material-icons-outlined text-blue-500 text-3xl">
            auto_awesome
          </i>
          Featured{" "}
                  ...
                  .map((item, index) => (
                    ...
                    >
                      <div className="w-full h-64 relative overflow-hidden">
                        {/* Background Gradient */}
                        <div
                          className={`absolute inset-0 bg-gradient-to-br ${item.gradient} flex items-center justify-center overflow-hidden`}
                        >
                          {/* Background Image */}
                          <img
                            alt={item.title}
                          ...
                          {item.hasCallout ? (
                            // Callout Banner
                            <div
                              className={`absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-10 w-[120%] h-48 ${item.calloutColor || "bg-yellow-400"} rounded-l-full shadow-lg flex flex-col justify-center pl-12 pr-24 rotate-[-3deg] group-hover:rotate-[-1deg] transition-transform duration-500`}
                            >
